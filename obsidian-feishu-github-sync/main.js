...
var GitHubService = class {
  ...
  async sync() {
    ...
    if (hasLocalChanges) {
      await this.commit(`Sync: ${(/* @__PURE__ */ new Date()).toISOString()}`);
    }
    await this.push();
    return {
...
var SyncManager = class {
  ...
  async syncAll() {
    ...
    try {
      ...
      new import_obsidian.Notice(`Sync failed: ${error}`);
    } finally {
      this.isSyncing = false;
    }
    return result;
...
var FeishuGitHubSyncPlugin = class extends import_obsidian2.Plugin {
  ...
  async onload() {
    ...
    new import_obsidian2.Notice("Feishu GitHub Sync loaded - Weekly sync on Monday at 9:00 AM");
  }
  onunload() {
  ...
  checkAndRunWeeklySync() {
    ...
    if (currentDay === this.settings.weeklySyncDay && currentHour === this.settings.weeklySyncHour && currentMinute === this.settings.weeklySyncMinute && this.lastSyncDate !== syncKey) {
      this.lastSyncDate = syncKey;
      new import_obsidian2.Notice("Starting weekly sync...");
      this.syncAll();
    }
  }
  async syncAll() {
    new import_obsidian2.Notice("Syncing Obsidian with Feishu and GitHub...");
    try {
  ...
  async syncFeishuOnly() {
    new import_obsidian2.Notice("Syncing with Feishu...");
    try {
      ...
      new import_obsidian2.Notice(`Feishu sync: ${total} files, ${errors} errors`);
      this.savePluginData();
    } catch (error) {
      new import_obsidian2.Notice(`Feishu sync failed: ${error}`);
    }
  }
  async pushToGitHub() {
    new import_obsidian2.Notice("Pushing to GitHub...");
    try {
      new import_obsidian2.Notice("Push complete");
    } catch (error) {
      new import_obsidian2.Notice(`Push failed: ${error}`);
    }
  }
  async pullFromGitHub() {
    new import_obsidian2.Notice("Pulling from GitHub...");
    try {
      new import_obsidian2.Notice("Pull complete");
    } catch (error) {
...
var FeishuSyncSettingsTab = class {
  ...
  display(containerEl) {
    ...
    const days = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
    days.forEach((day, i2) => {
      const opt = daySelect.createEl("option", { value: String(i2) });
      opt.text = day;
      if (i2 === settings.weeklySyncDay)
        opt.selected = true;
    });
